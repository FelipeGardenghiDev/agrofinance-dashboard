...
O **FeeAgroBank** entrega uma interface de internet banking corporativo que consolida:
...
2. **Liquidação e Transferências Instantâneas:** Módulo completo de transferências bancárias (PIX, TED) e aportes diretos em pools de liquidez RWA.
3. **Reatividade e Persistência em Tempo Real:** Débito imediato de saldo, cálculo automático de cotas tokenizadas adquiridas e atualização instantânea do extrato histórico.
4. **Governança & Compliance KYC:** Monitoramento de status cadastral de produtor rural e documentação regulatória (CPF, comprovante de produtor e biometria).
...
* **Tabela de Ativos RWA:** Detalhamento por tipo de grão, ticker do token (`SOJA24`, `SOJAO`, `MLHO25`, `MLHOP`), quantidade de tokens, preço unitário e variação 24h.
...
### 📜 3. Histórico de Transações & Filtros Avançados
...
│   │   ├── dashboard/                # Painel principal consolidado
...
│   │   │   └── features/             # Componentes de negócio (TransactionFilters, DetailModal)
│   ├── lib/
...
│       └── TransactionFilters.test.tsx # Teste de integração de componente React
...
O projeto conta com **24 testes automatizados** cobrindo regras de negócio críticas:
...
```text
 ✓ src/__tests__/store.test.ts (6 tests)
 ✓ src/__tests__/utils.test.ts (8 tests)
